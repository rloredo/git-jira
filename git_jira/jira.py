...
class JiraProject:
    ...
    def get_issues(self, status="Planned"):
        ...
        """
        return [
            JiraIssue(issue_key=i.raw["key"])
            for i in Jira().jira.search_issues(
                f"project = {self.project_code} AND status = '{status}' AND type != Epic"
            )
        ]
...
class JiraMetaIssue:
    def __init__(self, project_code=None):
        ...
        self.meta_issue = [
            p
            for p in Jira().jira.createmeta(expand="projects.issuetypes.fields")[
                "projects"
            ]
            if p["key"] == self.project_code
        ][0]
        self.issue_type_names = [
            issue["name"] for issue in self.meta_issue["issuetypes"]
        ]

    def available_fields(self, issue_type):
        fields = [
            i["fields"]
            for i in self.meta_issue["issuetypes"]
            if i["name"] == issue_type
        ][0]
            ...
            if field_metadata["type"] in ["option", "array"]:
                ...
                try:
                    field_metadata["options"] = [
                        {"value": opt["value"], "id": opt["id"]}
                        for opt in field["allowedValues"]
                    ]
                except:
    ...
    def required_fields(self, issue_type):
        return [
            field for field in self.available_fields(issue_type) if field["required"]
        ]
        ...
        if issue_key:
            try:
                ...
                self.available_statuses = [
                    {"display_name": status["to"]["name"], "name": status["name"]}
                    for status in Jira().jira.transitions(self.issue)
                    if status["isAvailable"] and status["to"]["name"] != self.status
                ]
                try:
