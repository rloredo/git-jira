...
def jira_config_input():
    config = dict()
    config['server_url'] = click.prompt("Insert your Jira server url", type=str)
    config['username'] = click.prompt("Insert your Jira username", type=str)
    config['api_token'] = click.prompt("Create an API token here https://support.atlassian.com/atlassian-account/docs/manage-api-tokens-for-your-atlassian-account/ \nand insert your Jira API token", type=str)
    config['default_project_key'] = click.prompt("Select default project to use. \nLeave empty if you want to specify it as flags. Insert the short code (like TP, PJ, etc)", type=str) 
    return config
